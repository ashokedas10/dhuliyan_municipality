...
$sql="SELECT * FROM `service` WHERE `SubCatId` =".$subcat->id; 
...
?> 
		 <table  border="0">
		  <tr>
			...
			<?php echo 'Title : '.$row->title;?><br /><?php 
			if($row->details<>''){echo 'Details :'.$row->details.'<br />';}?><?php if($row->doocument!=''){?>
			<a href="<?php echo $picpath.$row->doocument; ?>">click here</a><?php } ?>
