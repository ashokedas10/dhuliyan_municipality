...
class Authmod extends CI_Model {
	...
	function get_admin_info(){
		$admin_login = $this->session->userdata('admin_login');
			...
			$query = $this->db->query("SELECT * FROM tbl_party WHERE id = '$a_id' ");
			return $query->result();
		}
	}
	...
	function admin_get_login($data)
	{
		...
		$query = $this->db->query("SELECT `Email_Id`, `password` FROM tbl_party WHERE Email_Id = '$user_name' AND password = '$user_pass' ");
		    if ($query->num_rows() > 0)
			{
				...
				redirect('cmscontaint/', 'refresh');
				exit();
			}
			else
			{
				redirect('cmscontaint/party_login/', 'refresh');
				exit();
			}
	...
	function update_password($data){
		...
		$a_id = $this->session->userdata('a_id');
		$query = $this->db->query("SELECT `a_id`, `user_name`, `usertype` FROM p_webmaster WHERE a_id = '$a_id' AND user_pass = '$pass' AND  AND usertype = 3");
		if ($query->num_rows() > 0){
	...
	function category_list($pid=0){
		$sql = "SELECT * FROM p_category WHERE pid = $pid AND status = 'A' ";
		$query = $this->db->query($sql);
		$data['category_list'] = $query->result();
		return $data;
	}
