...
<div class="latest-heading"><span class="red-text">Public Works</span></div>
...
<ul class="nav nav-list bs-docs-sidenav">
  ...
  <?php 
   //cat 2 for administrator
    $sqlsubcat="SELECT * FROM `subcategory` WHERE `cat_id` =3 order by orderby "; 
...
$sql="SELECT * FROM `publicwork` WHERE `SubCatId` =".$subcat->id; 
...
<?php 
$picpath=BASE_PATH_ADMIN.'uploads/publicwork/';
$sql="SELECT * FROM `publicwork` WHERE `SubCatId` =".$subcat->id; 
$datalist = $this->projectmodel->get_records_from_sql($sql);
...
?> 
		 <table  border="0">
		  <tr>
			...
			if($row->details<>''){echo 'Details :'.$row->details.'<br />';}?><?php if($row->doocument!=''){?><a href="<?php echo $picpath.$row->doocument; ?>">click here</a><?php } ?></pre>
			</td>
		  </tr>
		</table>
		<p>&nbsp;</p>		
